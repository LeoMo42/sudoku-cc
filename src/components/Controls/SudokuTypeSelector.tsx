      ...
      {open && (
        <div className="absolute mt-1 w-full z-50">
          <div
            ref={listboxRef}
            id={listboxId}
            className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg max-h-64 overflow-y-auto"
            role="listbox"
          ...
          >
            {types.map((type) => (
              <button
                ...
                tabIndex={-1}
                className={`
                  w-full px-4 py-2.5 text-left transition-colors border-b border-gray-100 dark:border-gray-700 last:border-b-0
                  ${currentType === type.id
          ...
          {showScrollHint && (
            <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-white dark:from-gray-800 to-transparent rounded-b-lg pointer-events-none" aria-hidden="true" data-testid="scroll-gradient" />
          )}
        </div>
      )}
    </div>
